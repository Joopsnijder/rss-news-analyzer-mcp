...
class NewsAnalyzer:
    ...
    def analyze_trending_topics(self, hours: int = 24, min_mentions: int = 3) -> List[TrendData]:
        """
        ...
        try:
            ...
            for keyword, keyword_article_list in keyword_articles.items():
                if len(keyword_article_list) >= min_mentions:
                    ...
                    trend_score = self._calculate_trend_score(keyword_article_list, hours)
                    
                    # Get date range
                    article_dates = [article.published for article in keyword_article_list if article.published]
    ...
    def detect_news_spikes(self, hours: int = 24, comparison_hours: int = 168) -> List[NewsInsight]:
        """
        ...
        try:
            ...
            historical_articles = self.rss_service.get_all_recent_articles(comparison_hours)
                ...
                if historical_rate > 0:
                    ...
                    if spike_ratio > 2.0 and recent_count >= 3:  # At least 2x increase
                        insight = NewsInsight(
                            ...
                            evidence=[f"Spike ratio: {spike_ratio:.2f}x", f"Recent mentions: {recent_count}"],
        ...
        try:
            ...
            for trend in trends[:5]:  # Top 5 trends
                suggestion = {
                    ...
                    "suggested_angle": self._suggest_topic_angle(trend.keyword, trend.articles)
                }
            ...
            for spike in spikes[:3]:  # Top 3 spikes
                suggestion = {
                    ...
                    "urgency": "high",
                    "evidence": {"confidence": spike.confidence, "details": spike.evidence},
            ...
            suggestions.sort(key=lambda x: urgency_order.get(x["urgency"], 0), reverse=True)
        ...
        try:
            ...
            for keyword, count in keyword_count.most_common(limit):
                trending_keywords.append({
                    ...
                    "count": count,
                    "sources": list(keyword_sources[keyword]),
                    "trend_score": self._calculate_trend_score(keyword_articles[keyword], hours),
                    "sample_articles": [
    ...
    def _calculate_trend_score(self, articles: List[NewsArticle], hours: int) -> float:
        ...
        # Combine scores
        trend_score = (frequency_score * 0.4 + recency_score * 0.4 + source_diversity_score * 0.2)
    ...
    def _suggest_topic_angle(self, keyword: str, articles: List[NewsArticle]) -> str:
        ...
        return angles.get(keyword.lower(), f"Deep dive into {keyword}: trends, challenges, and opportunities")
