...
from ..utils.rss_utils import (
    fetch_rss_feed, extract_feed_metadata, extract_google_alerts_articles,
    extract_standard_rss_articles, is_google_alerts_feed, extract_news_keywords
)
...
class RSSFeedService:
    ...
    def fetch_feed(self, feed_id: str, force_refresh: bool = False) -> Optional[FeedData]:
        """
        ...
        try:
            ...
            # Fetch fresh data
            logger.info(f"Fetching fresh data for feed: {feed_id} (URL: {feed_config.url})")
            ...
            for article in feed_data.articles:
                try:
                    if not article.published:
                        logger.warning(f"Article has no published date: {article.title}")
                    ...
                    try:
                        ...
                        import email.utils
                        try:
                            timestamp = email.utils.parsedate_to_datetime(article.published)
                            article_date = timestamp
                        except:
                            # If all parsing fails, include the article if looking back more than 24 hours
                            logger.warning(f"Could not parse date '{article.published}' for article: {article.title}")
                            if hours > 24:
                                recent_articles.append(article)
                    ...
                    if article_date and article_date >= cutoff_time:
                        ...
                        hours_ago = (datetime.now() - article_date.replace(tzinfo=None)).total_seconds() / 3600
                        logger.debug(f"Article '{article.title}' is {hours_ago:.1f} hours old (cutoff: {hours})")
                        
                except Exception as e:
                    logger.warning(f"Error processing article date: {e} for article: {article.title}")
                    # Include articles with date errors if looking back more than 24 hours
                    if hours > 24:
                        recent_articles.append(article)
                    continue
            
            logger.info(f"Found {len(recent_articles)} recent articles (within {hours}h) from {len(feed_data.articles)} total, {date_parse_failures} date parse failures")
            return sorted(recent_articles, key=lambda x: x.published or '', reverse=True)
    ...
    def search_articles(self, query: str, feed_id: Optional[str] = None, 
                       hours: int = 168) -> List[NewsArticle]:
        """
        ...
        try:
            ...
            for article in articles:
                ...
                keyword_match = any(query_lower in keyword.lower() for keyword in article.keywords)
    ...
    def _process_feed_data(self, feed_config: FeedConfig, raw_feed: Dict[str, Any]) -> FeedData:
        """
        ...
        if feed_config.type == "google_alerts" or is_google_alerts_feed(feed_config.url):
    ...
    def _extract_keywords(self, article: Dict[str, Any], config_keywords: List[str]) -> List[str]:
        """
            ...
            for feed_config in enabled_feeds:
                try:
                    ...
                    if feed_data:
                        ...
                        results["failed"] += 1
                        results["errors"].append(f"Failed to refresh feed: {feed_config.id}")
