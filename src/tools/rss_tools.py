...
def register_rss_tools(mcp: FastMCP):
    ...
    def configure_rss_feeds() -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error getting RSS configuration: {str(e)}"}, indent=2)
    ...
    @mcp.tool()
    def fetch_rss_feed(feed_id: str, force_refresh: bool = False, debug: bool = False) -> str:
        """
        ...
        try:
            ...
            if not feed_config:
                return json.dumps({"error": f"Feed configuration not found: {feed_id}"}, indent=2)
    ...
    def analyze_news_trends(hours: int = 24, min_mentions: int = 3) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error analyzing news trends: {str(e)}"}, indent=2)
    ...
    def get_trending_keywords(hours: int = 24) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error getting trending keywords: {str(e)}"}, indent=2)
    ...
    def detect_news_spikes(hours: int = 24, comparison_hours: int = 168) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error detecting news spikes: {str(e)}"}, indent=2)
    ...
    def suggest_timely_topics(hours: int = 24) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error suggesting timely topics: {str(e)}"}, indent=2)
    ...
    def get_news_summary(hours: int = 24) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error generating news summary: {str(e)}"}, indent=2)
    
    @mcp.tool()
    def search_news_articles(query: str, feed_id: Optional[str] = None, hours: int = 168) -> str:
        """
        ...
        try:
            ...
            return json.dumps({"error": f"Error searching news articles: {str(e)}"}, indent=2)
    ...
    def get_company_mentions(hours: int = 24) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error analyzing company mentions: {str(e)}"}, indent=2)
    ...
    def get_feed_statistics(feed_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error getting feed statistics: {str(e)}"}, indent=2)
        ...
        try:
            ...
            try:
                ...
                if raw_feed:
                    ...
                    debug_info["feed_title"] = getattr(raw_feed.feed, 'title', 'No title')
                    debug_info["feed_description"] = getattr(raw_feed.feed, 'description', 'No description')
                    ...
                    if hasattr(raw_feed, 'entries') and raw_feed.entries:
                        ...
                        debug_info["sample_article"] = {
                            ...
                            "description": getattr(first_entry, 'description', 'No description')[:200]
                        }
            ...
            return json.dumps({"error": f"Error debugging RSS feed: {str(e)}"}, indent=2)
    ...
    def analyze_feed_keywords(feed_id: str, hours: int = 168) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Error analyzing feed keywords: {str(e)}"}, indent=2)
    ...
    def get_all_feed_articles(feed_id: str, limit: int = 20) -> str:
        ...
        try:
            ...
            if not feed_data:
                return json.dumps({"error": f"Could not fetch feed: {feed_id}"}, indent=2)
            ...
            return json.dumps({"error": f"Error getting all feed articles: {str(e)}"}, indent=2)